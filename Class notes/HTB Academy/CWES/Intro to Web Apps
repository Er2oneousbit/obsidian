...
![Dashboard showing 932 online players, 1032 machine owns, 622 challenge owns, and recent activity with targets Lame, Emdee five for life, and Delivery.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/htb-web-app_1.jpg)
...
![Comparison of website and web app: Website has static pages, same for everyone; Web app has dynamic, interactive pages, different for each user.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/website_vs_webapps.jpg)
...
![Introduction to Web Applications](https://academy.hackthebox.com/storage/modules/75/avatar.png)

[Introduction to Web Applications 47.06%](https://academy.hackthebox.com/app/module/75)
...
![Diagram showing a server connected to four clients: desktop, laptop, smartphone, and monitor.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/client-server-model.jpg)
...
![Diagram showing a server connected to three clients: smartphone, laptop, and monitor.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/one-server-arch.jpg)
...
![Diagram showing a database connected to three servers, each linked to a smartphone, laptop, and monitor client.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/many-server-one-db-arch.jpg)
...
![Diagram showing three databases connected to three servers, each linked to a smartphone, laptop, and monitor client.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/many-server-many-db-arch.jpg)
...
![ASP.NET Core architecture diagram showing client, IIS reverse proxy, ASP.NET Core web app with filters and services, connected to data sources, identity providers, and third-party services.](https://docs.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/media/image5-12.png)Source: [Microsoft Docs](https://docs.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/common-web-application-architectures)
...
![Browser window displaying HTML, CSS, and JS code snippets for a webpage layout.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/frontend-components.jpg)
...
![Diagram showing a back-end server running Linux and Windows, connected to a web server (Apache, NGINX, IIS), web application (PHP, C#, Java), and database (MySQL, MS SQL, Oracle).](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/backend-server.jpg)
...
This would display the following:![Browser window with page title 'Page Title', URL 'www.example.coml', heading 'A Heading', and paragraph 'A Paragraph'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/web_apps_html_2.jpg)
...
[Go to Questions](https://academy.hackthebox.com/app/module/75/section/756#questions-list)
...
![Screenshot of HTML and JavaScript code in a text editor.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/view_source1.png)
...
![Login form with fields for username and password, and a login button.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/web_apps_login_form_.png)
...
![Dialog box displaying a cookie value and a 'Close' button.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/web_apps_xss_2.jpg)
...
![Diagram showing a web server handling requests and responses from three clients: smartphone, laptop, and monitor.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/web-server-requests.jpg)
...
The following shows an example of requesting a page in a Linux terminal using the [cURL](https://en.wikipedia.org/wiki/CURL) utility, and receiving the server response while using the `-I` flag, which displays the headers:
        ...
        shellsession
`er2oneousbit@htb[/htb]$ curl -I https://academy.hackthebox.com HTTP/2 200 date: Tue, 15 Dec 2020 19:54:29 GMT content-type: text/html; charset=UTF-8 ...SNIP...`

While this `cURL` command example shows us the source code of the webpage:

        shellsession
`er2oneousbit@htb[/htb]$ curl https://academy.hackthebox.com <!doctype html> <html lang="en"> <head> <meta charset="utf-8" /> <title>Cyber Security Training : HTB Academy</title> <meta name="viewport" content="width=device-width, initial-scale=1.0">`
...
![Apache logo with a feather.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/apache.png)
...
![NGINX logo.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/nginx.png)
...
![Microsoft IIS logo.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/iis.png)
...
![Database schema with 'users' table showing IDs, usernames, first and last names, and 'posts' table showing post IDs, user IDs, dates, and content.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/web_apps_relational_db.jpg)
...
![Table of posts with IDs, dates, and content, showing key-value pairs](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/web_apps_non-relational_db.jpg)
...
[Go to Questions](https://academy.hackthebox.com/app/module/75/section/762#questions-list)
...
![API diagram with connections to cloud, user, shopping, video, audio, and key icons](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/api_examples.jpg)
...
![Vulnerability & Exploit Database page showing search results for 'WordPress' vulnerabilities, including CVE details and severity.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/rapid7-db.jpg)
...
![CVSS score metrics table showing base, temporal, and environmental score metrics with various impact and exploitability factors.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/75/cvssv3_calc.png)
